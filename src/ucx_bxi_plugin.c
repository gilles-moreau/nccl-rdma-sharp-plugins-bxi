...
// With ncclNet_v11_t the NCCL core initializes the network plugin
...
ncclResult_t nccl_ucx_get_properties_v10(int dev,
                                         ncclNetProperties_v10_t *props_v10) {
  ncclNetProperties_t props;
  ncclResult_t ret =
      nccl_p2p_ib_get_properties(ncclIbDevs, ncclNMergedIbDevs, dev, &props);
  if (ret != ncclSuccess)
    return ret;
...
ncclResult_t nccl_ucx_get_properties_v9(int dev,
...
ncclResult_t nccl_ucx_get_properties_v8(int dev,
...
ncclResult_t nccl_ucx_get_properties_v7(int dev,
...
ncclResult_t nccl_ucx_get_properties_v6(int dev,
...
static ncclResult_t ucx_init_context(ucp_context_h *ctx, int dev) {
  ...
  if (ucp_ctx[dev] == NULL) {
    ...
    snprintf(ucx_dev_name + strlen(ucx_dev_name),
             PATH_MAX - strlen(ucx_dev_name), ":%d", ncclIbDevs[dev].portNum);
...
ncclResult_t nccl_ucx_init(void **ctx, uint64_t commId,
                           ncclNetCommConfig_v11_t *config,
                           ncclDebugLogger_t logFunction,
                           ncclProfilerCallback_t profFunction) {
  if (ucxRefCount++)
    return ncclSuccess;
  if (ncclParamUCXDisable())
    return ncclInternalError;
...
ncclResult_t nccl_ucx_listen(void *ctx, int dev, void *handle,
  ...
  NCCLCHECK(ncclSocketInit(&comm->sock, &nccl_ucx_if_addr, my_handle->magic,
                           ncclSocketTypeNetIb, NULL, 1, 0));
  NCCLCHECK(ncclSocketListen(&comm->sock));
  NCCLCHECK(ncclSocketGetAddr(&comm->sock, &my_handle->connectAddr));
  NCCLCHECK(
      ucx_get_ctx_and_worker(dev, &comm->ctx, &comm->ucx_worker, &comm->tag));
...
ncclResult_t nccl_ucx_connect(void *ctx, int dev, void *handle,
  ...
  *send_comm = NULL;
  ...
  NCCLCHECK(ncclSocketInit(&comm->sock, &recv_handle->connectAddr,
                           recv_handle->magic, ncclSocketTypeNetIb, NULL, 1,
                           0));
  stage->comm = comm;
...
ucx_connect_check:
  ...
  NCCLCHECK(
      ucx_get_ctx_and_worker(dev, &comm->ctx, &comm->ucx_worker, &comm->ctag));
...
ncclResult_t nccl_ucx_accept(void *listen_comm, void **recv_comm,
  ...
  *recv_comm = NULL;
  ...
  NCCLCHECK(ncclSocketInit(&r_comm->sock, NULL, NCCL_SOCKET_MAGIC,
...
ucx_accept_check:
  ...
  if (r_comm->gpuFlush.enabled) {
    ...
    NCCLCHECK(ucx_worker_get_netaddress(r_comm->ucx_worker->worker, &my_addr,
                                        &local_addr_len));
    ep_params.field_mask = UCP_EP_PARAM_FIELD_REMOTE_ADDRESS;
...
ncclResult_t nccl_ucx_init_v10(ncclDebugLogger_t logFunction,
                               ncclProfilerCallback_t profilerCallback) {
  return nccl_ucx_init(NULL, 0, NULL, logFunction, NULL);
}

ncclResult_t nccl_ucx_connect_v10(int dev, ncclNetCommConfig_v10_t *config,
...
ncclResult_t nccl_ucx_listen_v10(int dev, void *opaqueHandle,
                                 void **listenComm) {
  return nccl_ucx_listen(NULL, dev, opaqueHandle, listenComm);
}

ncclResult_t nccl_ucx_connect_v9(int dev, void *opaqueHandle, void **sendComm,
                                 ncclNetDeviceHandle_t **sendDevComm) {
...
ncclResult_t nccl_ucx_isend_v9(void *sendComm, void *data, size_t size, int tag,
                               void *mhandle, void **request) {
  return nccl_ucx_isend(sendComm, data, size, tag, mhandle, NULL, request);
}

ncclResult_t nccl_ucx_irecv_v9(void *recvComm, int n, void **data,
...
ncclResult_t nccl_ucx_isend_v8(void *sendComm, void *data, int size, int tag,
                               void *mhandle, void **request) {
  return nccl_ucx_isend_v9(sendComm, data, (size_t)size, tag, mhandle, request);
}

ncclResult_t nccl_ucx_irecv_v8(void *recvComm, int n, void **data, int *sizes,
                               int *tags, void **mhandles, void **request) {
  size_t sizes_sizet[NCCL_NET_IB_MAX_RECVS];
  for (int i = 0; i < n; i++)
    sizes_sizet[i] = sizes[i];
  return nccl_ucx_irecv_v9(recvComm, n, data, sizes_sizet, tags, mhandles,
                           request);
}

ncclResult_t nccl_ucx_regmr_v7(void *comm, void *data, int size, int type,
                               void **mhandle) {
...
ncclNet_v11_t ucxPlugin_v11 = {.name = "UCX",
                               .init = nccl_ucx_init,
...
ncclNet_v10_t ucxPlugin_v10 = {.name = "UCX",
...
ncclNet_v9_t ucxPlugin_v9 = {.name = "UCX",
...
ncclNet_v6_t ucxPlugin_v6 = {.name = "UCX",
                             .init = nccl_ucx_init_v9,
                             .devices = nccl_ucx_devices,
